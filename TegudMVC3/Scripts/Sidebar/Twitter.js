define([
        ...
        var _getTweets = function (username, callback) {
            var url = 'http://api.twitter.com/1/statuses/user_timeline.json?include_entities=true&include_rts=true&screen_name=tegud&count=25&callback=?';
            console.log('Get new tweets (' + url + ')');
        ...
        var enableUI,
            ...
            toggleArticle = function (show, callback) {
                console.log((show ? 'Showing' : 'Hiding') + ' article');
            ...
            toggleLoading = function (show, callback) {
                // Hide the loading div if we're showing the loading one.
                if (show) {
                    console.log('Showing twitter loading screen');
                    ...
                    console.log('Hiding twitter loading screen');
        ...
        enableUI = function () {
            ...
            console.log('Twitter enableUI complete');
            twitterSetupComplete.resolve();
        };

        setTweetUI = function () {
            var tweetDateHeaders = $.map(tweets, function (item, i) {
            ...
            dates
                ...
                .html(tweetDateHeaders);

            console.log('Date header: ' + dates.size());
            console.log('Date header count: ' + dates.children().size());
            console.log('Date header html: ' + dates.html());
        ...
        $.when(TEGUD.pageLoadDeferred).then(enableUI);

        // The refresh function.
        (getTweets = function (showLoad) {
            if (TEGUD.pageLoadDeferred.state() !== 'pending' && (showLoad || typeof showLoad === 'undefined')) {
            ...
            _getTweets(user, function (data) {
                ...
                $.when(TEGUD.pageLoadDeferred, twitterSetupComplete).then(setTweetUI);
